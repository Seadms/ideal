...
import { todayString, timeInAppTz, cn } from '@/lib/utils'
...
// Server component: today's timeline from the calendar feed. Fetches are
// cached (revalidate 300) so this stays cheap.
export async function TodaySchedule() {
  if (!calendarConfigured()) {
    return (
      <Link href="/settings" className="flex items-center gap-3 rounded-xl border border-dashed border-zinc-800 px-4 py-3 text-zinc-500 hover:border-zinc-700 hover:text-zinc-400 transition-colors">
        ...
        <p className="text-xs">Connect your calendar to see your day here. Set up in Settings.</p>
      </Link>
    )
  }
  ...
  const events = await getCalendarEvents(new Date(now.getTime() - 12 * 3600_000), dayEnd)
